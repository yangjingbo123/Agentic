...
class Blackboard:
    ...
    # flaw 窗口（v3.2：80 → 300 → **600**，当前值跟随 `MAX_CHANNEL_CHARS`，见那里的
    # 第十轮注释）。80 是 v3.1 拍的，当时没量过 critic 到底说多长。
    # 实测（v2+v3 SFT 数据，剥离 <interaction> 块后）：critic **真报了错**的 218 条
    # 输出中位 267 字、p75 341 字、p90 494 字，完整送达率 80→4%、300→63.3%、
    # 600→96.3%。
    ...
    # ↑ **这句在第十轮被推翻了，但保留下来当病灶记录。** 它的算术没错（p90 确实
    # 508、边际收益确实递减），错的是把预算当成零和：当时没人去数**同一个 prompt
    # 里有多少字是重复的**。第十轮量出来 310 处「对方内容」有 247 处是同 prompt 内
    # 逐字重复、每处约 263 字，去重腾出的空间比窗口 300→600 要花的还多。教训是
    # "放宽收益递减"这个判断只在"预算已经花在有用的东西上"这个前提下成立，而那个
    # 前提从来没被验证过。
    #
