...
def get_custom_reward_fn(config):
    ...
    import sys
    sys.modules["custom_module"] = module  # make it picklable by pebble subprocesses
    try:
...
def run_ppo(config) -> None:
    ...
    if not ray.is_initialized():
        ray.init(
            # If RAY_ADDRESS is set (by the launch script which pre-started a
            # cluster with the correct conda Python), connect to it.
            # Otherwise fall back to starting a local cluster.
