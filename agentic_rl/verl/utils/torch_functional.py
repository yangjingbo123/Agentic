...
def logprobs_from_logits(logits, labels):
    ...
    if FLAH_ATTN_CROSS_ENTROPY_LOSS_AVAILABLE:
        ...
        # No flash_attn: use logsumexp trick — vectorized, memory-efficient, no full softmax materialization
        # log_softmax(x)[i] = x[i] - logsumexp(x)
        safe_labels = labels.clone()
        safe_labels[safe_labels == -100] = 0
        logits_at_label = torch.gather(logits, dim=-1, index=safe_labels.unsqueeze(-1)).squeeze(-1)
        logsumexp = torch.logsumexp(logits, dim=-1)
        output = logits_at_label - logsumexp
    return output
