...
For each training step:
  ...
  ▌ Phase 2: Rewards
  r_prop  ← p^i_t
  r_int   ← 交互因果矩阵（§4.2）；forced 轮发起方数值为 0，且**不进入 int 优势通道**
...
4. **forced 只在 u=0 时注入**；被注入轮发起方总奖励中的 r_int 数值为 0，
   但 `r_int_w=None`、**完全退出 int anchor group**，响应方正常计分；
   **eval 时 ε=0**（评测学到的策略本身）。注意：在组相对优势中，数值 0 仍会
   参与 μ/σ 并获得正/负优势，不能代表“不奖不罚”；只有通道缺席才是零交互梯度。
...
2. **主 turn 双通道优势**（raca_adv）：历史 v2.3 写成
   `z(r_prop，不分层) + z(λ·r_int，p_t 分层)`，恢复了解题信号并保留 r_int 隔离；
   int_rate=0 时 prop 通道仍供梯度，修复的是**解题能力停训**，不是交互决策吸收态。
   08-28 两跑实测 int_rate 到 0 后 160 步不恢复；`v32_miss` 也只延迟到 step25。
   **第十五轮再修两层信用混叠**：① `z(λr)=z(r)` 使任何正 λ 失效，改为
   `A_sol=z(r_prop)`、`A_int=λ·z(r_int)`；② 旧版先把两优势相加再广播整个 response，
   正确答案与错误求助会互相抵消。现按 token span 路由：A_sol 只训练推理+答案，
   A_int 只训练末尾 `<interaction>` 块；forced 仅有 A_sol。
