...
class RayPPOTrainer(object):
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            sample_questions.extend(test_batch.non_tensor_batch['question'].tolist())
            sample_groundtruths.extend(
                [x['ground_truth'] for x in test_batch.non_tensor_batch['reward_model'].tolist()]
            )
        ...
        if self.config.trainer.get('save_val_generations', False):
            ...
            results_to_save = []
            for q, g, r, s in zip(sample_questions, sample_groundtruths, sample_outputs, sample_scores):
    ...
    def _save_train_generations(self, batch: DataProto):
        ...
        for i, data_item in enumerate(batch):
            ...
            output_texts = self.tokenizer.decode(output_ids, skip_special_tokens=True)
            results_dict[uid]['response'].append(output_texts)
            results_dict[uid]['score'].append(
                data_item.batch['token_level_rewards'].sum().item()
            )
