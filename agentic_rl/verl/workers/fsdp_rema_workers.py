...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(self,
        ...
        # ── Multi-LoRA: inject adapters BEFORE FSDP wrap ──────────────────
        _use_lora = (role == 'actor' and
                     hasattr(self.config.model, 'lora_rank') and
                     int(self.config.model.get('lora_rank', 0)) > 0)
        if _use_lora:
            from peft import LoraConfig, get_peft_model
            lora_cfg = LoraConfig(
                r=int(self.config.model.lora_rank),
                lora_alpha=int(self.config.model.get('lora_alpha', self.config.model.lora_rank * 2)),
                target_modules=list(self.config.model.get('lora_target_modules', ['q_proj', 'v_proj'])),
                lora_dropout=0.0,
                bias='none',
                task_type='CAUSAL_LM',
            )
            adapter_names = list(self.config.model.lora_adapters)
            actor_module = get_peft_model(actor_module, lora_cfg, adapter_name=adapter_names[0])
            for name in adapter_names[1:]:
                actor_module.add_adapter(name, lora_cfg)
            # Freeze base weights; only lora_ params remain trainable
            for name, param in actor_module.named_parameters():
                param.requires_grad_('lora_' in name)
            if self.rank == 0:
                actor_module.print_trainable_parameters()
            log_gpu_memory_usage('After multi-LoRA injection', logger=logger)
        # ──────────────────────────────────────────────────────────────────
        ...
        actor_module_fsdp = FSDP(
            ...
            use_orig_params=_use_lora,  # True required for PEFT multi-adapter compatibility
        ...
        if role == 'actor' and optim_config is not None:
            ...
            # When using LoRA, only optimize the trainable adapter params (much smaller set)
            opt_params = ([p for p in actor_module_fsdp.parameters() if p.requires_grad]
                          if _use_lora else list(actor_module_fsdp.parameters()))
    ...
    def _build_rollout(self):
        ...
        if rollout_name == 'hf':
        ...
        elif rollout_name == 'vllm_blackboard':
            from verl.workers.rollout.vllm_rollout.vllm_rollout_blackboard import VLLMBlackboardRollout
            from verl.workers.sharding_manager import BaseShardingManager
            log_gpu_memory_usage('Before building vllm_blackboard rollout', logger=None)
            local_path = copy_to_local(self.config.model.path)
            rollout = VLLMBlackboardRollout(
            ...
            class BlackboardShardingManager(BaseShardingManager):
                """Sleep vLLM after rollout, wake up before rollout. No weight sync (LoRA managed separately)."""
                ...
                def __exit__(self, *args):
                    self.engine.sleep(level=1)  # level=1: offload weights to CPU, discard KV cache
                    torch.cuda.empty_cache()

            rollout_sharding_manager = BlackboardShardingManager(rollout.inference_engine)
            log_gpu_memory_usage('After building vllm_blackboard rollout', logger=None)

        elif rollout_name == 'sglang':
            from verl.workers.rollout.sglang_rollout import SGLangRollout
            # NOTE(linjunrong): importing SGLang symbols checks CUDA capability; do it here
            # not in sharding_manager.__init__ to avoid "No CUDA GPUs" on the Ray main process.
